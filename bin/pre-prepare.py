...
if __name__ == '__main__':
	...
	partition = partitionClusters(options.number, clusters)
	...
	for cluster in trainClusters:
		...
		if not os.path.exists(options.output + "twatter-train/" + clusterName):
			os.makedirs(options.output + "twatter-train/" + clusterName)

		# Find each ID and put the tweet into a new file in the new directory
		for tweetID in cluster[1]:
			if os.path.exists(options.input + "/" + str(tweetID)):
				...
				newTweetFile = open(str(options.output + "/twatter-train/" + clusterName + "/" + str(tweetID)), 'w')
				newTweetFile.write(line)
				tweetFile.close()
				newTweetFile.close()
	...
	for cluster in testClusters:
		...
		# Create a directory for this cluster
		if not os.path.exists(options.output + "twatter-test/" + clusterName):
			os.makedirs(options.output + "twatter-test/" + clusterName)
		...
		for tweetID in cluster[1]:
			if os.path.exists(options.input + "/" + str(tweetID)):
				...
				tweetFile = open(options.input + "/" + str(tweetID))
				line = tweetFile.readline();
				newTweetFile = open(str(options.output + "/twatter-test/" + clusterName + "/" + str(tweetID)), 'w')
				newTweetFile.write(line)
				tweetFile.close()
				newTweetFile.close()
